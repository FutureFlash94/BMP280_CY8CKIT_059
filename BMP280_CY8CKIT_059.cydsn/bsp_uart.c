...
*
* Description : Puts a string with measured pressure and temperature in the format "P:value-T:value"
*               into the transmit buffer to be sent when the bus is available. This is a blocking 
...
CPU_VOID uart_send_press_temp(Bmp280_press_temp bmp280_data)
{
    ...
    tx_msg_len = strlen(strcpy((CPU_CHAR *)tx_msg, "P:"));
    tx_msg_len += snprintf((CPU_CHAR *)(tx_msg+tx_msg_len), 15, "%d-", (CPU_INT32U)bmp280_data.press);
    tx_msg_len += strlen(strcpy((CPU_CHAR *)(tx_msg+tx_msg_len), "T:"));
    tx_msg_len += snprintf((CPU_CHAR *)(tx_msg+tx_msg_len), 15, "%d.%2d\n", (CPU_INT32S)temp, (CPU_INT32S)(100*temp)%100);
